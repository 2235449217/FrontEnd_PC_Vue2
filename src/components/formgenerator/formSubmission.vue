...
import { getEmptyForm } from '@/api/formGenerator';
...
export default {
  ...
  methods: {
    getForm() {
      console.log(this.appid, '5264564854815111');
      getEmptyForm(this.appid)
        .then((res) => {
          console.log(this.appid, '52645648548151');
          this.formData = JSON.parse(res.data);
          console.log(res.data, '我是getEmptyForm');
          // 显示表单
          this.onOpen();
        })
        .catch((error) => {
          console.log(error, '我是getEmptyFormError');
        });
    },
    onOpen() {
