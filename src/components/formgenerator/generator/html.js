// 【二、Vue代码生成器】第119行（搜索关键词：const tags = {}）
// 这个页面（html.js）中的代码都是字符串拼接处理(同样，js.js也是通过字符串拼接的方式生成的脚本文件)，并不高深，如需进一步处理可以从入口函数makeUpHtml顺着结构阅读源码。
...
// 下面这些配置完之后就可以点击{运行}按钮预览了。（原来的页面代码是通过json格式展示的，点击运行后是通过HTML代码展示的）
const tags = {
  'el-test': el => {
    // 这是el-button常用的属性（attrBuilder会生成常用的属性）（attrBuilder是公共的Vue组件属性。const修饰的是可以选择设置的Vue组件属性）
    const {
      tag, disabled
    } = attrBuilder(el);
    // 剩下的这些属性都是el-button组件属性对应的，目标是生成字符串'<el-button type="success" icon="el-icon-warning" size="medium">主要按钮</el-button>'
    ...
    // 这个方法是用来获取__slot__.default的，也就是获取按钮内的文字的。因为：__slot__:{ default:'主要按钮'}
  ...
  'el-button': el => {
    // 这是el-button常用的属性（attrBuilder会生成常用的属性）
    const {
      tag, disabled
    } = attrBuilder(el);
    // 剩下的这些属性都是el-button组件属性对应的，目标是生成字符串'<el-button type="success" icon="el-icon-warning" size="medium">主要按钮</el-button>'
    ...
    const plain = el.plain ? 'plain' : '';
    const circle = el.circle ? 'circle' : '';
    // 这个方法是用来获取__slot__.default的，也就是获取按钮内的文字的。因为：__slot__:{ default:'主要按钮'}（准确来说，buildElButtonChild是来获取__config__,__slot__里的自定义属性的。上面是el组件的本身的属性。）
    let child = buildElButtonChild(el);
  ...
  'el-checkbox-group': el => {
    // attrBuilder:专门设置tag,vModel,clearable,placeholder,width,disabled这些属性。用于<${tag} ${vModel} ${min} ${max} ${size} ${disabled}>${child}</${tag}>`生成Vue组件
    const { tag, disabled, vModel } = attrBuilder(el);
    const size = `size="${el.size}"`;
    const min = el.min ? `:min="${el.min}"` : '';
    const max = el.max ? `:max="${el.max}"` : '';
...
function buildElSelectChild(scheme) {
  // console.log('我是scheme');
  const children = [];
  const slot = scheme.__slot__;
  // console.log(scheme.__slot__, '我是scheme');
  // console.log(scheme.__vModel__, '我是scheme.__vModel__');
...
function buildElCheckboxGroupChild(scheme) {
  // console.log(scheme, '我是html.js里的sceme');
  const children = [];
  const slot = scheme.__slot__;
  const config = scheme.__config__;
  if (slot && slot.options && slot.options.length) {
