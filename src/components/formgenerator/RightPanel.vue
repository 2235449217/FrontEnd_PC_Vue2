// 【第三步】右侧el组件属性的可视化：src/views/index/RightPanel.vue  例：增加{按钮类型}第264行代码（关键词查：按钮类型）
    //  增加{组件尺寸}第495行代码（关键词查：组件尺寸  ||或运算）
    // 增加{按钮图标} 第277行代码 （关键词查：按钮图标  ||或运算）
    // type:组件类型，size:组件尺寸 icon:el-icon-search
    <!-- activeData：就是config.js中每一条json -->
<template>
  <div class="right-board">
    <!-- 右上角【组件属性】【表单属性】 -->
    <el-tabs
      v-model="currentTab"
      class="center-tabs my-tabs"
    >
      <el-tab-pane
        label="组件属性"
        name="field"
...
import TreeNodeDialog from '@/components/formgenerator/TreeNodeDialog';
...
.right-board ::v-deep .el-tabs__item {
  width: 175px;
}
.right-board ::v-deep.el-tabs__active-bar {
  width: 175px;
}
</style>
